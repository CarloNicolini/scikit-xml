...
SparseLike = csr_array | csr_matrix | sp.sparray
ArrayLike = np.typing.ArrayLike
SparseOrDenseLike = SparseLike | ArrayLike
...
def compute_inv_propensity(labels: SparseLike, A: float, B: float) -> np.ndarray:
    ...
    frequencies = np.ravel(labels.sum(axis=0))
    C = (np.log(num_instances) - 1) * np.power(B + 1, A)
    wts = 1.0 + C * np.power(frequencies + B, -A)
    return np.ravel(wts)
...
def _top_k(
    ...
    val: np.ndarray, optional
        topk val; size=(num_rows, k)
    """
    ind, val = _topk_numba(X.data, X.indices, X.indptr, k, pad_ind, pad_val)
    if return_values:
...
def _get_topk_from_sparse(X: SparseLike, pad_indx: int = 0, k=5):
    """
    ...
    X.sort_indices()
    pad_indx = X.shape[1]
    indices = _top_k(X, k, pad_indx, 0, return_values=False)
    return indices


def _get_topk_from_array(X: ArrayLike, k: int = 5, sort_values: bool = False):
    """
    ...
    # values are given
        ...
        raise ValueError("Only floating or integer dtype supported")


def _get_topk_from_dict(X: dict, k: int = 5, sort_values: bool = False):
    """
    ...
    # get top-k entries without sorting them
    if scores.shape[1] > k:
...
def _get_topk(X: SparseOrDenseLike, pad_indx: int = 0, k=5, sort_values: bool = False):
    ...
    Parameters
    ----------
    X: scipy sparse or numpy dense array
    pad_indx: int

    k: int
    sort_values: bool
        Sort the values or not

    Returns
    -------
    """
    if sp.issparse(X):
        indices = _get_topk_from_sparse(X=X, pad_indx=pad_indx, k=k)
    elif isinstance(X, np.ndarray):
        indices = _get_topk_from_array(X=X, k=k, sort_values=sort_values)
    elif isinstance(X, dict):
        indices = _get_topk_from_dict(X=X, k=k, sort_values=sort_values)
    else:
...
def _setup_metric(
    ...
    inv_psp: np.ndarray | None = None,
    k: int = 5,
    sort_values: bool = False,
) -> tuple[
    SparseOrDenseLike | None,
    SparseOrDenseLike | None,
    SparseOrDenseLike | None,
    SparseOrDenseLike | None,
]:
    """
    Setup the metric for evaluation

    Parameters
    ----------
    X: scipy sparse or numpy dense array
        Prediction matrix
    true_labels: scipy sparse or numpy dense array
        Ground truth matrix
    inv_psp: np.ndarray, optional
    ...
    Returns
    -------
    tuple: indices, true_labels, ps_indices, inv_psp
        indices: Top-k indices
        true_labels: Ground truth matrix with an additional column
        ps_indices: Top-k indices for propensity scored precision
        inv_psp: Updated propensity scores
    """
...
def _compute_eval_flags(
    indices: SparseOrDenseLike,
    true_labels: SparseOrDenseLike,
    inv_psp: ArrayLike | None = None,
) -> ArrayLike:
    """
    Compute evaluation flags based on the input data.

    Parameters
    ----------
    indices: scipy sparse or numpy dense array
        Input data
    true_labels: scipy sparse or numpy dense array
        Target values
    inv_psp: array-like of shape (n_samples,), default=None
        Inverse propensity scores
    Returns
    -------
    array: eval_flags
        Evaluation flags based on the input data
    """
    if sp.issparse(true_labels):
        nr, nc = indices.shape
        rows = np.repeat(np.arange(nr).reshape(-1, 1), nc)
        eval_flags = true_labels[rows, indices.ravel()].ravel().reshape(nr, nc)
        if inv_psp is not None:
            return np.multiply(inv_psp[indices], eval_flags)
        return eval_flags
        ...
        if inv_psp is not None:
            return np.multiply(inv_psp[indices], eval_flags)
        return eval_flags
    raise TypeError(f"Not a supported type for true_labels {type(true_labels)}")
...
def _precision_helper(X, true_labels, k=5, sort_values=False):
    """
    ...
    eval_flags = _compute_eval_flags(indices, true_labels, None)
    return _precision(eval_flags, k)


def _psprecision_helper(X, true_labels, inv_psp, k=5, sort_values=False) -> float:
    """
    ...
    eval_flags = _compute_eval_flags(indices, true_labels, inv_psp)
    ps_eval_flags = _compute_eval_flags(ps_indices, true_labels, inv_psp)
...
def _ndcg_helper(X, true_labels, k=5, sort_values=False):
    """
...
def _psndcg_helper(X, true_labels, inv_psp, k=5, sort_values=False):
    """
...
def _recall(eval_flags, deno, k=5):
    recall = np.mean(np.multiply(np.cumsum(eval_flags, axis=-1), deno), axis=0)
    return np.ravel(recall)


def _recall_helper(X, true_labels, k=5, sort_values=False):
    """
...
def _hits_helper(X, true_labels, k=5, sort_values=False):
    """
...
def _psrecall_helper(X, true_labels, inv_psp, k=5, sort_values=False):
    """
...
def _auc_helper(X, true_labels, k, sort_values=False):
    """
    Compute AUC score
